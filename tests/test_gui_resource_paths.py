...
def test_legacy_gui_timer_alias_was_removed():
    source = Path("Python/new_ui.py").read_text(encoding="utf-8")
    assert "spawn_vehicle_and_cameras" not in source
    assert "self.timer = QTimer()" not in source
    assert "self.timer = self.startTimer" not in source
...
def test_stop_only_requests_cancellation_until_worker_cleanup_finishes():
    ...
    finished_source = _method_source("_simulation_worker_finished")
