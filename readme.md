...
into a plain thread-safe queue that is drained by a GUI-owned Qt timer; no Qt object or
bound signal enters a worker or CARLA callback thread. All `QImage`, `QPixmap`,
progress-bar, and widget updates happen on the GUI thread. Stop is cooperative: controls remain in a
